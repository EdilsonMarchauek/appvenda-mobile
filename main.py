...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(
                f"https://aplicativovendashash-e42a2-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-e42a2-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco)==float) and (type(quantidade)==float):
            ...
            requests.post(f"https://aplicativovendashash-e42a2-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",
                          data=info)
            ...
            requisicao = requests.get(f"https://aplicativovendashash-e42a2-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
            total_vendas = float(requisicao.json())
            total_vendas += preco
            info = f'{{"total_vendas": "{total_vendas}"}}'
            requests.patch(f"https://aplicativovendashash-e42a2-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                           data=info)
            homepage = self.root.ids["homepage"]
            homepage.ids["label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] [b]R${total_vendas}[/b]"
            self.mudar_tela("homepage")
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(
            f'https://aplicativovendashash-e42a2-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
    ...
    def carregar_vendas_vendedor(self, dic_info_vendedor, *args):
        ...
        try:
            ...
            pagina_vendaoutrovendedor = self.root.ids["vendasoutrovendedorpage"]
            lista_vendas = pagina_vendaoutrovendedor.ids["lista_vendas"]
        ...
        pagina_vendaoutrovendedor.ids["label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] [b]R${total_vendas}[/b]"
